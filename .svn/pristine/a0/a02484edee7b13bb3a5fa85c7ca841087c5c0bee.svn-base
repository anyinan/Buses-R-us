...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private void newStopClusterer() {
        ...
        int radius = zoom == 0 ? 125 : 1000 / zoom;
            ...
            for (Route r : s.getRoutes()) {
                ...
                for (RoutePattern rp : r.getPatterns()) {
                    ...
                    for (int i = 0; i < path.size() - 2; i ++ ) {
                        LatLon pt1 = path.get(i);
                        LatLon pt2 = path.get(i + 1);
                        LatLon pt3 = path.get(i + 2);
                        if (Geometry.rectangleIntersectsLine(northWest, southEast, pt1, pt2)) {
                            points.add(new GeoPoint(pt1.getLatitude(), pt1.getLongitude()));
                            points.add(new GeoPoint(pt2.getLatitude(), pt2.getLongitude()));
                        }
                        points.add(new GeoPoint(pt3.getLatitude() , pt3.getLongitude()));
                    }
        ...
        for (Stop s : StopManager.getInstance()){
            ...
            GeoPoint geoPoint = new GeoPoint(locn.getLatitude() , locn.getLongitude());
                ...
                if (Geometry.rectangleContainsPoint(northWest , southEast , locn))  {
                    if (nearestStopMarker != null && nearestStopMarker.getPosition().equals(geoPoint)){
                        nearestStopMarker.setRelatedObject(s);
                        setMarker(s , nearestStopMarker);
                        ...
                        String routeInfo = "" + System.lineSeparator();
                        for (Route r : s.getRoutes()){
                            routeInfo = routeInfo + r.getNumber() + System.lineSeparator();
                        }
                        marker.setTitle(  s.getNumber()+ System.lineSeparator() + s.getName()+ routeInfo);
                        setMarker(s , marker);
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        if (nearest != null){
            List<Marker> markers = stopClusterer.getItems();
            for (Marker mk : markers){
                if (mk.getRelatedObject() == nearest){
                    nearestStopMarker = mk;
    ...
    private void handleLocationChange(Location location) {
        ...
        LatLon loca = new LatLon(lat , lon);
        centerAt(new GeoPoint(location.getLatitude(),location.getLongitude()));
        Stop nearest = StopManager.findNearestTo(loca);
        updateMarkerOfNearest(nearest);
        markStops();
        locationListener.onLocationChanged(nearest , loca);
